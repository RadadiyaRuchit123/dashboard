    ...
    <div className="space-y-8 animate-in fade-in duration-700 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-3 no-print-area">
          <button onClick={exportAllToPDF} className="flex items-center gap-2 px-6 py-2.5 border rounded-xl font-black text-sm hover:bg-zinc-50 dark:hover:bg-white dark:hover:text-black transition-all shadow-sm" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)', color: 'var(--text-primary)' }}><Download size={18} />Export All PDF</button>
          <button onClick={() => window.print()} className="flex items-center gap-2 px-6 py-2.5 border rounded-xl font-black text-sm hover:bg-zinc-50 dark:hover:bg-white dark:hover:text-black transition-all shadow-sm" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)', color: 'var(--text-primary)' }}><Printer size={18} />Print</button>
      ...
      <div className="premium-card rounded-[32px] overflow-hidden" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
        <div className="p-8 border-b flex flex-col md:flex-row md:items-center justify-between gap-6 no-print-area" style={{ borderColor: 'var(--border-color)' }}>
          <div className="flex items-center gap-4">
            <button className="flex items-center gap-2 px-4 py-2.5 border rounded-xl font-bold text-sm hover:bg-zinc-50 dark:hover:bg-white dark:hover:text-black transition-all shadow-sm" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)', color: 'var(--text-primary)' }}><Filter size={18} />Filter</button>
            <div className="relative flex-1 md:w-80"><Search className="absolute left-4 top-1/2 -translate-y-1/2" size={18} style={{ color: 'var(--text-muted)' }} /><input type="text" placeholder="Search now" className="w-full pl-12 pr-4 py-2.5 border rounded-xl focus:outline-none focus:border-brand-blue transition-all font-medium text-sm shadow-sm" style={{ backgroundColor: 'var(--bg-input)', borderColor: 'var(--border-color)', color: 'var(--text-primary)' }} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /></div>
          </div>
        </div>
            ...
            <tbody className="divide-y" style={{ borderColor: 'var(--border-color)' }}>
              {currentItems.map((order, idx) => (
                <tr key={idx} className="hover:bg-white transition-all duration-300 group cursor-pointer border-b" style={{ borderColor: 'var(--border-color)' }}>
                  <td className="px-8 py-5 text-xs font-bold transition-all group-hover:!text-black group-hover:!opacity-100" style={{ color: 'var(--text-muted)' }}>{order.id}</td>
                  <td className="px-8 py-5"><div className="flex items-center gap-3 transition-all group-hover:!text-black"><div className="w-10 h-10 rounded-xl overflow-hidden bg-zinc-100 border p-1" style={{ borderColor: 'var(--border-color)' }}><img src={order.image} alt="" className="w-full h-full object-cover rounded-lg" /></div><span className="text-xs font-black">{order.product}</span></div></td>
                  <td className="px-8 py-5 text-xs font-black transition-all group-hover:!text-black" style={{ color: 'var(--text-primary)' }}>{order.qty.toString().padStart(2, '0')}</td>
                  <td className="px-8 py-5 text-xs font-bold transition-all group-hover:!text-black group-hover:!opacity-100" style={{ color: 'var(--text-muted)' }}>{order.date}</td>
                  <td className="px-8 py-5"><div className="flex items-center gap-2"><div className="w-7 h-7 rounded-full bg-zinc-200 border-2 border-white dark:border-zinc-800 overflow-hidden shadow-sm"><img src={`https://i.pravatar.cc/150?u=${order.customer}`} alt="" className="w-full h-full object-cover" /></div><span className="text-xs font-bold transition-all group-hover:!text-black" style={{ color: 'var(--text-primary)' }}>{order.customer}</span></div></td>
                  <td className="px-8 py-5 text-xs font-black transition-all group-hover:!text-black" style={{ color: 'var(--text-primary)' }}>{currency}{order.price?.toString().replace(/[₹$]/g, '')}</td>
                  <td className="px-8 py-5 text-xs font-bold transition-all group-hover:!text-black group-hover:!opacity-100" style={{ color: 'var(--text-muted)' }}>{order.payment}</td>
