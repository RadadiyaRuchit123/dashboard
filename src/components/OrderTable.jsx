...
function DeleteModal({ order, onConfirm, onClose }) {
  return (
    <Backdrop onClose={onClose}>
      <div style={{ background:'var(--bg-card)', borderRadius:28, width:'100%', maxWidth:420, boxShadow:'0 32px 80px rgba(0,0,0,0.15)', overflow:'hidden', textAlign:'center' }}>
        <div style={{ padding:'32px 28px 20px' }}>
          ...
          <p style={{ fontSize:18, fontWeight:800, color:'var(--text-primary)', margin:'0 0 8px' }}>Delete Order?</p>
          <p style={{ fontSize:13, color:'var(--text-muted)', lineHeight:1.6, margin:0 }}>
            You are about to delete <br />
            <strong style={{ color:'var(--text-primary)' }}>{order.product}</strong> ({order.displayId}).<br />
            This action cannot be undone.
          </p>
        </div>
        <div style={{ padding:'0 28px 28px', display:'flex', gap:12 }}>
          <button onClick={onClose} style={{ flex:1, padding:'12px', borderRadius:16, border:'1.5px solid var(--border-color)', background:'transparent', color:'var(--text-secondary)', fontWeight:700, fontSize:14, cursor:'pointer' }}>
            Cancel
          </button>
          <button onClick={() => { onConfirm(order.id); onClose(); }}
            style={{ flex:1, padding:'12px', borderRadius:16, border:'none', background:'linear-gradient(135deg,#ef4444,#dc2626)', color:'#fff', fontWeight:700, fontSize:14, cursor:'pointer', boxShadow:'0 4px 14px rgba(239,68,68,0.35)' }}>
            Yes, Delete
          </button>
        </div>
      </div>
